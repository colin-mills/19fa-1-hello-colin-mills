...
test_case = {
  ...
  'description' : 'The "Hello.java" in your repository compiles',
  'points_possible' : 10,
  'points_earned' : 0,
  'test_ran' : False,
  'test_passed' : False,
...
def do_test( test_case, skip_remaining_tests ):
  ...
  print('Running "javac Hello.java"...')
  test_timeout = False
  # $ javac Hello.java
  proc_compile = Popen( ["javac", "Hello.java"],
                        stdout=PIPE,
                        stderr=STDOUT,
                        text=True )
  ...
  # test returned
    ...
    if ( proc_compile.returncode != 0 ):
      ...
      test_case['feedback'] = 'Compiler error on "Hello.java"'
      skip_remaining_tests = True
    # Compiled!
    else:
  ...
  if ( skip_remaining_tests ):
    print('Must have a compiled program to continue; skipping remaining tests...')
